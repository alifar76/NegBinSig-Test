...
zinb_nb_test <- function(both,MYdata,trt){
  all_data <- foreach(i=2:(length(rownames(MYdata))+1), .combine = rbind) %dopar% {
    ...
    final_vec <- c(final_vec,mean_diff,pval_ttest,shap_wilk_pval,heading,estimate_tab[1][[1]],names(estimate_tab)[1],estimate_tab[2][[1]],names(estimate_tab)[2],kwtest)
    final_vec
  }
  return (all_data)
}
...
final_steps <- function(otutable,mapfile,categ1,categ2,trt,outputname){
  ...
  all_data <- cbind(otuids,all_data[,1:2],zinb.qval,all_data[,3:4],nb.qval,all_data[,9],all_data[,11],all_data[,5:6],ttest.qval,all_data[,13],kw.qval,all_data[,7],taxlabels)
  colnames(all_data) <- c("OTU_IDs","ZINB_Coeff","ZINB_pval","ZINB_qval","NB_Coeff","NB_pval","NB_qval",mean1_head,mean2_head,difflabel,"ttest_pval","ttest_qval","KW_pval","KW_qval","Shapiro_Wilk_Normality_pvalue","taxonomy") #change Difference to groups being tested
