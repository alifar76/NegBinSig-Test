...
def filter_results(iname,outname,qval,filtinds,qvals):
	...
	for line in infile:
		...
		if line.startswith('OTU_IDs'):
			# Model name, OTU ID, Mean Difference, Significant Group, Lowest BIC value, p-value for selected model, taxonomy
			outfile.write("Selected Model"+"\t"+spline[0]+"\t"+spline[12]+"\t"+"Significant Group"+"\t"+"BIC value of Selected Model (Lowest)"+"\t"+"p-value for selected model"+"\t"+spline[28]+"\t"+'\t'.join(spline[18:24])+"\n")		
			...
			if spline[filtinds[0]:filtinds[1]] != ['NA','NA','NA']:									# Indices of BIC values of Poisson, NB and ZINB with outliers filtered/not filtered
				...
				if spline[qvals[2]] != 'NaN':
					if (indexic == 0) and (Decimal(spline[qvals[0]]) < qval):									# Filter OTUs with Poisson p-value less than specified cutoff
				...
				if spline[qvals[2]] != 'NaN':
					if (indexic == 1) and (Decimal(spline[qvals[1]]) < qval):									# Filter OTUs with NB p-value less than specified cutoff
				...
				if spline[qvals[2]] != 'NaN':
					if (indexic == 2) and (Decimal(spline[qvals[2]]) < qval):									# Filter OTUs with ZINB p-value less than specified cutoff
...
pvalnonfilt = [2,5,8]		# Indices of p-values of 3 models with outliers not filtered
filter_results("NegBinSig-Test_EXP_GROUP_L4.txt","NegBinSig-Test_EXP_GROUP_L4_FILTERED.txt",1.00,notfilt,pvalnonfilt)
